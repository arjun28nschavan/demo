...
Test.prototype.text = function (statusExpected, textExpected) {
    var self = this
    return this
        .headers({'content-type': 'text/html; charset=utf-8'})
        .payload(statusExpected, textExpected)
}
    ...
    if (!test.reqCtx.printed) {
        ...
        if (test.reqCtx.req) {
            message += (prefix ? ' ' : '')
                ...
                + (test.reqCtx.req.payload ? ':\t' + JSON.stringify(test.reqCtx.req.payload) : '')
        }
        if (test.reqCtx.res) {
            message += '\n' + Test.indent + '  '
